...
分支按依赖全部集成后，不自动清理：向用户简要列出已合并分支与残留 worktree，询问是否清理；用户确认后用 `worktree_flow.py remove --branch … --target … --delete-branch` 删除对应 worktree 目录、变空的 `.worktrees` 父目录和分支引用。用户未确认、清理失败或分支未合并时保留并说明，不得用 `--force` 丢弃有未合并成果的工作区。不默认推送、强推或重写已发布历史；已有对应明确授权时无需再问。
