...
WINDOWS_SCRIPTS = ("start.bat", "stop.bat")
...
PROFILE_FRAGMENTS = {
    ...
    "node": {"start_cmd": 'nohup npm --prefix "$SCRIPT_DIR" run start', "start_note": "TODO(project): confirm package.json defines a start script.", "bat_start_cmd": "npm run start", "build_hint": "npm ci && npm run build"},
    "jvm": {"start_cmd": 'nohup java -jar "$SCRIPT_DIR/app.jar"', "start_note": "TODO(project): point at the real runnable JAR.", "bat_start_cmd": "java -jar app.jar", "build_hint": "mvn -DskipTests package or ./gradlew bootJar"},
    "binary": {"start_cmd": 'nohup "$SCRIPT_DIR/app"', "start_note": "TODO(project): point at the real built executable.", "bat_start_cmd": "app.exe", "build_hint": "go build -o app ./... or cargo build --release"},
    "generic": {"start_cmd": 'nohup "$SCRIPT_DIR/run"', "start_note": "TODO(project): no service stack was detected; replace this with the real start command.", "bat_start_cmd": "app.exe", "build_hint": "the repository's verified build command"},
}
