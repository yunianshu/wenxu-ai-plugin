...
WINDOWS_SCRIPTS = ("dev.bat", "start.bat", "stop.bat")
...
PROFILE_FRAGMENTS = {
    ...
    "node": {"start_cmd": 'nohup npm --prefix "$SCRIPT_DIR" run start', "start_note": "TODO(project): confirm package.json defines a start script.", "bat_start_cmd": "npm run start", "bat_debug_cmd": "call npm run start", "debug_note": "TODO(project): confirm package.json defines a start script; local debugging usually prefers its dev/watch script.", "build_hint": "npm ci && npm run build"},
    "jvm": {"start_cmd": 'nohup java -jar "$SCRIPT_DIR/app.jar"', "start_note": "TODO(project): point at the real runnable JAR.", "bat_start_cmd": "java -jar app.jar", "bat_debug_cmd": "java -jar app.jar", "debug_note": "TODO(project): point at the real runnable JAR; append debug options (JVM debug agent, profile flags) here.", "build_hint": "mvn -DskipTests package or ./gradlew bootJar"},
    "binary": {"start_cmd": 'nohup "$SCRIPT_DIR/app"', "start_note": "TODO(project): point at the real built executable.", "bat_start_cmd": "app.exe", "bat_debug_cmd": "app.exe", "debug_note": "TODO(project): point at the real built executable; append debug flags here.", "build_hint": "go build -o app ./... or cargo build --release"},
    "generic": {"start_cmd": 'nohup "$SCRIPT_DIR/run"', "start_note": "TODO(project): no service stack was detected; replace this with the real start command.", "bat_start_cmd": "app.exe", "bat_debug_cmd": "app.exe", "debug_note": "TODO(project): no service stack was detected; replace this with the real local debug command.", "build_hint": "the repository's verified build command"},
}

DEV_BAT_TEMPLATE = """@echo off
setlocal EnableExtensions
rem Generated by ai-project-steward scaffold -- review every TODO(project) marker before deployment.
cd /d "%~dp0"

echo [dev] starting in local debug mode -- logs stream to this window; press Ctrl+C or close it to stop
echo [dev] this script writes no app.pid; use start.bat/stop.bat for the managed service lifecycle

rem TODO(project): replace the command below with the real local debug command (dev/watch script, debug flags, profiles).
rem __DEBUG_NOTE__
__DEBUG_CMD__
set "DEV_EXIT=%errorlevel%"
echo [dev] exited with code %DEV_EXIT%
pause
exit /b %DEV_EXIT%
"""

DOCKER_DEV_BAT = """@echo off
setlocal EnableExtensions
rem Generated by ai-project-steward scaffold -- review every TODO(project) marker before deployment.
cd /d "%~dp0"
if not defined COMPOSE_FILE set "COMPOSE_FILE=%~dp0docker-compose.yml"
if not exist "%COMPOSE_FILE%" ( echo [dev] ERROR: compose file not found: %COMPOSE_FILE% & pause & exit /b 1 )
rem TODO(project): confirm the compose file and services; use a compose override file for debug ports or profiles.
echo [dev] starting docker compose in the foreground -- press Ctrl+C to stop
...
START_BAT_TEMPLATE = """@echo off
setlocal EnableExtensions
rem Generated by ai-project-steward scaffold -- review every TODO(project) marker before deployment.
cd /d "%~dp0"

if exist app.pid for /f "usebackq delims=" %%i in ("app.pid") do (
  powershell -NoProfile -Command "if (Get-Process -Id %%i -ErrorAction SilentlyContinue) { exit 0 } else { exit 1 }" && ( echo [start] already running, PID %%i & exit /b 0 )
)

rem TODO(project): replace the command below with the real start command or service manager (NSSM, Task Scheduler, PM2, Windows service, ...).
rem __START_NOTE__
for /f "usebackq delims=" %%i in (`powershell -NoProfile -Command "(Start-Process -FilePath 'cmd.exe' -ArgumentList '/c','__START_CMD__ > run.log 2>&1' -WindowStyle Hidden -PassThru).Id"`) do >app.pid echo %%i

if not exist app.pid ( echo [start] ERROR: failed to start & pause & exit /b 1 )
for /f "usebackq delims=" %%i in ("app.pid") do echo [start] started, PID %%i

rem TODO(project): verify readiness against the real health endpoint.
if defined HEALTH_URL (
  powershell -NoProfile -Command "for($i=0;$i -lt 30;$i++){try{Invoke-WebRequest -UseBasicParsing $env:HEALTH_URL|Out-Null;exit 0}catch{Start-Sleep -Seconds 2}};exit 1"
  if errorlevel 1 ( echo [start] ERROR: service did not become healthy at %HEALTH_URL% & pause & exit /b 1 )
...
if not exist app.pid ( echo [stop] not running & exit /b 0 )
for /f "usebackq delims=" %%i in ("app.pid") do set "PID=%%i"
powershell -NoProfile -Command "if (Get-Process -Id %PID% -ErrorAction SilentlyContinue) { exit 0 } else { exit 1 }" || ( echo [stop] not running & del app.pid >nul 2>&1 & exit /b 0 )
rem TODO(project): stop through the real service manager; the sequence below is a graceful-then-forced fallback, escalate only when configured.
taskkill /PID %PID% /T >nul 2>&1
powershell -NoProfile -Command "Start-Sleep -Seconds 5" >nul
powershell -NoProfile -Command "if (Get-Process -Id %PID% -ErrorAction SilentlyContinue) { exit 0 } else { exit 1 }" && taskkill /PID %PID% /T /F >nul 2>&1
del app.pid >nul 2>&1
echo [stop] stopped, PID %PID%
"""

DOCKER_START_BAT = """@echo off
setlocal EnableExtensions
rem Generated by ai-project-steward scaffold -- review every TODO(project) marker before deployment.
cd /d "%~dp0"
if not defined COMPOSE_FILE set "COMPOSE_FILE=%~dp0docker-compose.yml"
if not exist "%COMPOSE_FILE%" ( echo [start] ERROR: compose file not found: %COMPOSE_FILE% & pause & exit /b 1 )
rem TODO(project): confirm the compose file, service names, and pinned image digests before deployment.
docker compose -f "%COMPOSE_FILE%" up -d
if errorlevel 1 ( echo [start] ERROR: docker compose up failed & pause & exit /b 1 )
rem TODO(project): verify service health (HTTP endpoint), not only container state.
docker compose -f "%COMPOSE_FILE%" ps
"""

DOCKER_STOP_BAT = """@echo off
setlocal EnableExtensions
rem Generated by ai-project-steward scaffold -- review every TODO(project) marker before deployment.
cd /d "%~dp0"
if not defined COMPOSE_FILE set "COMPOSE_FILE=%~dp0docker-compose.yml"
if not exist "%COMPOSE_FILE%" ( echo [stop] ERROR: compose file not found: %COMPOSE_FILE% & pause & exit /b 1 )
rem TODO(project): confirm the shutdown model -- named data volumes must survive 'down'.
docker compose -f "%COMPOSE_FILE%" down
if errorlevel 1 ( echo [stop] ERROR: docker compose down failed & pause & exit /b 1 )
"""
...
def render_script(name: str, profile: str) -> str:
    ...
    if name == "dev.bat":
        if profile == "docker": return DOCKER_DEV_BAT
        return DEV_BAT_TEMPLATE.replace("__DEBUG_NOTE__", fragment["debug_note"]).replace("__DEBUG_CMD__", fragment["bat_debug_cmd"])
    if name == "start.bat":
        if profile == "docker": return DOCKER_START_BAT
        return START_BAT_TEMPLATE.replace("__START_NOTE__", fragment["start_note"]).replace("__START_CMD__", fragment["bat_start_cmd"])
    if name == "stop.bat": return DOCKER_STOP_BAT if profile == "docker" else STOP_BAT_TEMPLATE
...
def ensure_scripts(root: Path, profile: str) -> list[str]:
    created = []
    for name in REQUIRED_SCRIPTS + WINDOWS_SCRIPTS:
        path = root / name
        if path.exists(): continue
        body = render_script(name, profile)
        write_crlf(path, body) if name.endswith(".bat") else write_lf(path, body)
        created.append(name)
    return created
...
def scaffold(root: Path, requested_version: str | None) -> dict:
    ...
    existing = [name for name in REQUIRED_SCRIPTS + WINDOWS_SCRIPTS if name not in created]
    version_info = detect_versions(root)
    ...
    todo_count = sum((root / name).read_text(encoding="utf-8").count("TODO(project)") for name in REQUIRED_SCRIPTS + WINDOWS_SCRIPTS if (root / name).exists())
